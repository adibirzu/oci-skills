...
# (common.sh: audit_log + run_mutating/confirm wiring). Verifies that a dry-run
...
OCI_SKILLS_AUDIT_LOG="$ledger" OCI_AUTH_MODE=config OCI_SKILLS_DRY_RUN=true \
  run_mutating "delete synthetic bucket" oci os bucket delete --name synthetic
...
assert rec["event"] == "mutating_dry_run", rec
...
OCI_SKILLS_AUDIT_LOG="$ledger2" OCI_SKILLS_NO_AUDIT=1 OCI_AUTH_MODE=config OCI_SKILLS_DRY_RUN=true \
  run_mutating "noop" oci os ns get
